...
Defines how user-facing copy is localized across the application: a single `vue-i18n` instance with Traditional Chinese as the default and fallback locale; interface copy resolved from locale resources rather than strings embedded in templates; behavioral state (tabs, modes, filters) keyed on language-neutral identifiers so switching locale never changes behavior; backend-provided names, user input, and persistent business identifiers preserved verbatim while their surrounding labels translate; and reusable components falling back to locale-backed defaults without overriding explicit caller-provided values.

Note: this spec is written in English because the original change was authored that way. New specs in this project use Traditional Chinese prose with English normative keywords (SHALL / SHALL NOT) — see `openspec/config.yaml`.
