...
### 警示列歸屬（2026-08-21 由 node `13:2` 確認）

警示列 `row_warn`（`275:3079`，352x44）**位於 `footer_sticky` 之內**，與 `row_cta` 同屬釘底區，不隨內容捲動。目前實作把它放在 `.video__sticky` 裡是正確的。

五個狀態的 `panel_config` 一律 400x823，但 footer 高度隨警示列存在與否改變：

| 狀態            | node       | scroll_area | footer_sticky | scroll_fade top | scrollbar_hint |
| --------------- | ---------- | ----------- | ------------- | --------------- | -------------- |
| MV-04 初始      | `13:2`     | 690         | **133**       | 662             | 552            |
| MV-04b 確認彈窗 | `125:681`  | 745         | 78            | 717             | 644            |
| MV-04c 生成中   | `491:9008` | 745         | 78            | 717             | 644            |
| MV-04d 完成     | `491:9337` | 745         | 78            | 717             | 644            |
| MV-04e 任務中心 | `491:9594` | 745         | 78            | 717             | 644            |

差值一致：133 減 78 等於 55、745 減 690 也是 55，即警示列 44 加間距 11。目前實作 `.video__sticky` 的 `gap: 0.6875rem`（11px）與設計稿相符。

**結論：警示列只在初始狀態出現，送出生成後即消失，footer 收合為 78。** 捲動區與遮罩位置要隨之改變，不能寫死。
