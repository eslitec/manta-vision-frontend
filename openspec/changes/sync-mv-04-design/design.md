...
- **生成模型（標準×1／進階×2／專業×4）新增為 `GenerateVideoView.vue` 的第 4 步，`VideoJobReq` 增加 `modelTier` 欄位，`createVideoJob` 依倍率計算 cost（45 × 倍率）。** 這是設計稿裡原本完全沒有的一步（比對首頁截圖時沒看到，是這次確認生成彈窗畫面才第一次出現），倍率選項與費率定義成 `types/api.ts` 的常數陣列 `VIDEO_MODEL_TIERS`，`mock.ts` 跟頁面元件共用同一份定義，避免兩邊各寫一套倍率數字。
...
（目前已無未解決的問題；上一輪的兩個問題已在 Decisions 中定案：背景任務資料放在全域 Pinia store；任務中心面板同時涵蓋影片與圖片生成任務。）
