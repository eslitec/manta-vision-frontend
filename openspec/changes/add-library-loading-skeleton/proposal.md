...
- 修正單頁伺服器分頁的檢視（資料夾、`aiGenerate`／`edit`／`video` 分類）在翻頁／切換分類時骨架屏完全不觸發的問題：查詢重打時 `assets.value` 沒有被清空，切頁後切出來的舊資料（切換前那一頁的內容）不是空的，導致「畫面目前沒有素材」這個判斷式一直是假，骨架屏因此不會出現，新資料回來時會整批瞬間跳成別的內容而沒有經過載入中的過渡。改成這類檢視查詢一開始就清空 `assets.value`
- **（決策 8，覆蓋前一版原本的做法）**「全部素材」「物件素材」「未分類」這幾個整批預先撈取、前端自行切頁的合流檢視，改成翻頁或切換分類時也一律固定觸發骨架屏——不再因為資料已經在本地快取（或目標頁面本來就有內建素材可以顯示）就跳過骨架屏。這是使用者實際使用後明確要求的取捨：視覺一致性（每次切換都看得到統一的載入過場）優先於「資料已快取就秒開」，代價是這幾個合流檢視翻頁會固定變慢到骨架屏的最短顯示時間（500ms），不再是原本的即時切換
