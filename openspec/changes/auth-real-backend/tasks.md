...
- [x] 1.1 對齊 Requirement「資料來源由環境變數切換且預設為假資料」、落地設計決策「`VITE_USE_MOCK` 預設為假資料」：`src/api/index.ts` 依 `VITE_USE_MOCK === 'false'` 在 `realApi` 與 `mockApi` 之間選擇；`src/env.d.ts` 宣告用到的環境變數型別，打錯字不會靜靜地變成 `undefined`
...
- [x] 2.1 對齊 Requirement「登入與註冊改由真後端驗證」、落地設計決策「`expiresAt` 存絕對時間，不存 `expiresIn`」：新增 `src/api/real.ts`，`login()` 打 `POST /auth/login`，把 `expiresIn`（秒）換算成 `expiresAt`（絕對毫秒）
...
- [x] 2.4 落地設計決策「用展開而不是分支」：`realApi` 以 `{ ...mockApi, login, register, logout }` 組成——後端尚未實作的 29 支端點沿用假資料，不會變成 `undefined`
...
- [x] 3.3 對齊 Requirement「憑證失效時把使用者送回登入頁」、落地設計決策「把處理器抽成 `sessionGuard.ts`」：把處理器從 `main.ts` 抽成 `src/api/sessionGuard.ts` 的 `installSessionGuard(session, router)`——`main.ts` 一 import 就會 `mount()`，寫在那裡等於測不到
- [x] 3.4 落地設計決策「`SESSION_INVALID_CODES` 不含 `INVALID_CREDENTIALS`」：`src/api/sessionGuard.spec.ts` 5 個測試：`TOKEN_INVALID`／`TOKEN_EXPIRED` 會清狀態並導頁帶 redirect、`INVALID_CREDENTIALS` 不會、已在登入頁不再導一次、`NETWORK_ERROR` 不登出
...
- [x] 4.2 落地設計決策「登入頁的 `throw e` 改成顯示訊息」：`LoginView.vue` 的 `showError: boolean` 改成 `errorMessage: string`，帳密錯誤以外的失敗（後端沒開、逾時、CORS 被擋）也顯示訊息，取代原本的 `throw e`
