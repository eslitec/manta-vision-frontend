...
- [x] 10.1 側邊欄品牌頭像與頂欄使用者頭像改為 `#606692`
- [x] 10.2 統計卡分隔線置中於 48px 項目間距，統計卡上方間距 22px → 16px
- [x] 10.3 以 1366×940 重新截圖比對 MV-00；內容區無整頁捲軸，兩個頭像與分隔線樣式符合校正值

## 11. 2026-08-21 側邊欄依 Figma sidebar_l 逐格校正

- [x] 11.1 對齊 Requirement「側邊欄標示目前所在區塊」：使用中導覽項目的左側強調色條由 20px 改為設計稿的 32px。**這覆蓋任務 8.5／9.8「刻意保留 20px」的決定**——當時是憑「跟文字／圖示等高」推斷，現在取到 `sidebar_l`（node `216:1246`）的 `accent_bar` 節點實際值為 `h-[32px]`（品牌區塊則是 38px，實作本來就是 38 沒錯）。同時補上使用中項目右側 10px 圓角（設計稿 `rounded-br-[10px] rounded-tr-[10px]`），與品牌區塊一致
- [x] 11.2 對齊 Requirement「側邊欄提供功能性連結」：底部「教學文件」「登出」依設計稿 `footer` 重做——每項高 48（原本靠 padding 撐出約 27）、左右內距 16、文字 14px Medium `#606692`（原為 13px `rgba(255,255,255,.35)`）、footer 下留白 24。任務 3.1「視覺弱化、未啟用」的意圖不變：`#606692` 落在 `#2e3567` 底上本來就是低對比，只是改用設計稿的精確色與尺寸，仍未加點擊行為
- [x] 11.3 導覽項目高度固定為設計稿的 48px（原本是 `padding: 13px 12px` 撐出來的高度）、項目間距 4px → 0（設計稿 `menu` 內項目是連續排列）、文字色 `rgba(255,255,255,.85)` → 純白、補行高 22
- [x] 11.4 補上品牌區塊與導覽之間的分隔線（設計稿 `divider`，node `216:727`：全寬 1px、`rgba(239,242,250,0.5)`），實作原本完全沒有這個元素
- [x] 11.5 `logo_wrap` 內距對齊設計稿：上 20／下 12（原本是 `.sidebar` 的 20 再加 `.sidebar__logo` 自己的 4，等於上 24、下 4），並補上行高 24；品牌名稱色 `$blue-dark-300`（#171e52）→ `$blue-dark-500`（#2e3567）、副標行高改用絕對值 20px
- [x] 11.6 以 Playwright 於 1366x940 實測：側邊欄 200x940、`logo_wrap` 36 高、品牌區塊 58 高、分隔線 200x1、導覽項目 200x48、底部連結 200x48，皆與設計稿相符
