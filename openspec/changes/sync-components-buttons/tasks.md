...
- [x] 1.1 建立 `AppButton.vue` 與六種 Figma variant。
- [x] 1.2 實作 hover、pressed、focus、disabled 與 loading 狀態。
- [x] 1.3 支援一般、緊湊與帶圖示尺寸。

## 2. 逐頁對照與替換

- [x] 2.1 對照 Figma 元件庫 `btn`（node `170:13`）的 type 與狀態。
- [x] 2.2 替換首頁、生成頁、圖庫、品牌設定、用量與編輯器內符合 `btn` 定義的操作。
- [x] 2.3 保留 chip、tab、option card、icon-only、toolbar 與純文字操作。
- [x] 2.4 移除頁面內重複的按鈕顏色、圓角與互動狀態覆寫。
