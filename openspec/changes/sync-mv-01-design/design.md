...
- **「我的資料夾」選取狀態改成虛線邊框樣式（`#EEF1F7` 底色＋`1.5px` 虛線邊框），只套用在 `--folder` 這個變體，不動「全部素材」／系統分類原本的純色高亮。** 一開始從截圖肉眼觀察，誤以為虛線是「拖曳中的放置目標」提示（因為資料夾清單下方剛好有「可拖曳」的說明文字），使用者後來提供精確 SVG 才確認這其實就是「已選取」的樣式，不是拖放提示——這是這輪比對中一個「肉眼猜測方向錯誤，靠精確截圖才糾正」的例子，跟 MV-00 階段反覆出現的教訓一致。邊框加上後為了不讓外框尺寸跳動，`padding` 對應減少邊框寬度（`9px 12px` → `7.5px 10.5px`），這樣切換選取狀態時排版不會抖動。
