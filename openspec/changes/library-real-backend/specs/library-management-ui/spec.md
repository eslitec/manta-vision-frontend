...
> **2026-08-28 補記**：底下四個 Requirement 是本 change 原本定案的 delta，已在
> `feat/library-real-backend-wiring`（commit `c83a3fe`）套用進正式的
> `openspec/specs/library-management-ui/spec.md`。實作時另外發現「左側篩選分成全部素材、
> 系統分類與我的資料夾三段」這個既有 Requirement 從未真的描述過決策 1 講的「未分類固定置頂」
> 行為（程式碼原本也真的漏刻了），一併補進正式 spec，delta 補在最後一節。
