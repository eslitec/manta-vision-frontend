...
> **2026-08-28 更新（實作已完成）**：對應的串接 change（`feat/library-real-backend-wiring`
> 分支，commit `c83a3fe`）已經落地，實作時直接讀了後端當時的原始碼（`app/schemas/folder.py`、
> `app/schemas/image.py`、`app/routers/images.py` 等），比這份文件寫成當下依據的 `docs/api.md`
> 更新——底下標了「已修正」的地方是文件當初的假設跟後端實際落地不一樣的地方，不是實作偏離了
> 這份文件。開放問題 B 也已經由實作直接向後端原始碼求證後定案，見該節更新。
...
> **已修正（實作時對照後端原始碼）**：`FolderResponse` 的欄位是 `folderName`，不是 `name`；
> 而且後端明確表示**不存在「系統資料夾」概念**，`FolderResponse` 沒有 `isSystem` 這個欄位。
> 下面的 `Folder` 型別已經按實際落地的樣子改過。
...
> **實作細節補充**：實際落地時「組裝」發生在 `LibraryView.vue` 的畫面層（側欄 template 裡手動
> 補一個 `FolderRow`，`folderId` 傳 `null`），不是在 `useAssets.ts` 把 `folders` 這個 ref
> 本身變成含未分類項目的陣列——`folders.value` 仍然只有使用者自訂的真實資料夾，`unfiledCount`
> 是分開的一個 ref。這代表任何其他要顯示資料夾清單的地方（例如未來若圖庫選圖彈窗要加資料夾篩選）
> 都要自己補這個組裝步驟，不能只讀 `folders`。目前確認過 `ImagePickerDialog.vue` 沒有依資料夾
> 篩選的 UI，不受影響。
...
> **已修正（實作時對照後端原始碼）**：後端 `pageSize` 實際預設值是 10，不是 24——不影響底下的
> 決策本身（前端本來就決定不依賴後端預設值，一律明確帶 `pageSize=8`），只是這裡的數字寫錯了。
> `counts` 不受篩選影響這件事已經對照後端 `count_by_bucket` 的 docstring 確認過，包含 `folderId`
> 與關鍵字搜尋在內都不影響，不是只有「與目前 page 無關」這麼窄。
...
> **2026-08-28 已定案（實作時直接對照後端原始碼確認語意一一對應）**：`app/schemas/image.py`／
> `app/services/images.py` 確認 `mediaType` 與 `source` 就是兩個獨立維度，`count_by_bucket`
> 的分桶規則是：`video` 這個媒體型態一律優先於 `source`（不管 `source` 是什麼，只要
> `mediaType === 'video'` 就算進 `video` 桶）；`tryon` 這個 `source` 值沒有自己的桶，併入
> `aiGenerate` 桶。前端採第一個選項：`Asset` 拆成 `type: MediaType` ＋ `source: AssetSource`
> 兩個獨立欄位（見 `types/asset.ts`），`AssetTag` 這個合併型別已經拿掉；`CATEGORY_TAGS` 用
> `dimension: 'source' | 'mediaType'` 標記每個系統分類該比對哪個欄位，避免把 `video` 誤當成
> `source` 送給後端查詢。這題不用再問後端，已經是實作驗證過的結論。
