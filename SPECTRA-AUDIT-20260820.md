...
基準：commit `952058d`（`spectra update` 產物已提交）
前次健檢：`SPECTRA-CHANGE-CHECKLIST.md`（2026-08-10，已過期）

## 一句話結論

Spectra 的「工具面」今天補齊了（設定開啟、13 個 skills 與 11 個 commands 已產生並提交），但「流程面」有三個結構性問題還沒解決：**正式規格基線是空的**、**換行符汙染讓所有 diff 與 drift 比對失真**、**14 個 change 幾乎全部完成卻沒有一個歸檔**。
...
### 2. 換行符全面 CRLF 汙染，diff 與 drift 都會失真
...
### 5. `openspec/config.yaml` 的專案上下文完全是空的
...
1. `.gitattributes` + `git add --renormalize .`，單獨一個 commit（解除 P0-2）
2. 跑上面四個 spectra 指令，把輸出貼回來
3. 補 `openspec/config.yaml` 的 context（P1-5）
4. 決定 `openspec-*` / `opsx` 是否移除（P1-4）
5. 用 `/spectra-ingest` 把字型選單改動補進 `sync-mv-09-design`（P2-9）
6. 收掉三個未完成任務（P2-8）
7. PR 合併後依序 validate → analyze → drift → archive
